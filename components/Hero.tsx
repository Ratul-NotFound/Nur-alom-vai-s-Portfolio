...
const Hstyle = "text-white font-bold text-3xl sm:text-4xl lg:text-5xl";

export const Hero: React.FC = () => {
  const stats = [
    { count: "1+", label: "YEARS OF EXPERIENCE" },
    { count: "4+", label: "FEATURED PROJECTS" },
    { count: "01", label: "IEEE PUBLICATION" },
    { count: "3.63", label: "B.SC. CSE CGPA" },
  ];

  return (
    <section id="home" data-name="Intro" className="scroll-mt-28 flex flex-col w-full gap-7 sm:gap-10 items-start pt-6 sm:pt-0">
      ...
      {/* Stats - Responsive 2x2 on small mobile, row on tablet/desktop */}
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap w-full gap-5 sm:gap-12 lg:gap-16">
        {stats.map((stat, i) => (
          <div key={i} className="flex flex-col items-start group cursor-default">
            ...
            <div className="max-w-[130px] sm:max-w-none text-left">
              <p className="text-gray-500 text-[11px] sm:text-xs md:text-sm font-semibold tracking-wider uppercase mt-1">
                {stat.label}
              </p>
            </div>
          </div>
      ...
      {/* Stack Feature Cards */}
      <div className="flex gap-5 sm:gap-6 flex-col md:flex-row w-full mt-2">
        {/* Card 1: AI & XAI */}
        <a
          href="#research"
          className="group relative overflow-hidden min-h-[17rem] md:h-72 w-full md:w-[48%] bg-[#1c1b19]/40 backdrop-blur-xl border border-white/[0.05] rounded-[2rem] p-7 sm:p-10 flex flex-col justify-between transition-all duration-500 hover:border-orange-500/40 hover:bg-orange-500/[0.03] hover:shadow-[0_20px_40px_-15px_rgba(249,115,22,0.15)]"
        >
          ...
          <div className="p-3.5 sm:p-4 bg-orange-500/10 rounded-2xl w-fit border border-orange-500/20 text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition-all duration-500">
            <FaBrain className="text-2xl sm:text-3xl" />
          </div>

          <div className="relative z-10 my-3">
            <h2 className="text-white font-bold text-lg sm:text-[1.3rem] pt-2 leading-tight">
        ...
        <a
          href="#projects"
          className="group relative overflow-hidden min-h-[17rem] md:h-72 w-full md:w-[52%] bg-[#1c1b19]/40 backdrop-blur-xl border border-white/[0.05] rounded-[2rem] p-7 sm:p-10 flex flex-col justify-between transition-all duration-500 hover:border-green-400/40 hover:bg-green-400/[0.03] hover:shadow-[0_20px_40px_-15px_rgba(74,222,128,0.15)]"
        >
          ...
          <div className="p-3.5 sm:p-4 bg-green-400/10 rounded-2xl w-fit border border-green-400/20 text-green-500 group-hover:bg-green-400 group-hover:text-black transition-all duration-500">
