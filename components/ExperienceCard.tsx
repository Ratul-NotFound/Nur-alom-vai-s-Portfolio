...
export const ExperienceCard: React.FC<ExperienceCardProps> = ({
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] group border border-transparent transition-all duration-500 hover:bg-white/[0.03] hover:border-white/[0.08] mb-3 backdrop-blur-sm relative overflow-hidden">
      ...
      <div
        className={`absolute -inset-1 bg-gradient-to-r from-orange-500/10 to-transparent ${
      ...
      {/* Icon badge */}
      <div
        className={`shrink-0 p-3 sm:p-4 rounded-xl sm:rounded-2xl ${
          active ? "bg-orange-500" : "bg-orange-500/10"
        } ${
          active ? "text-white" : "text-orange-500"
        } border border-orange-500/20 group-hover:bg-orange-500 group-hover:text-white transition-all duration-500 shadow-[0_0_20px_rgba(249,115,22,0.1)]`}
      >
      ...
      <div className="flex-1 space-y-2 sm:space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-white text-xl sm:text-2xl tracking-tight transition-colors duration-300 group-hover:text-orange-500">
            {heading}
          </h3>
