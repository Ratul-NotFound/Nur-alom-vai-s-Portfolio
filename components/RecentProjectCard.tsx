  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-52 h-44 sm:h-48 md:h-38 shrink-0 relative z-10 overflow-hidden rounded-xl sm:rounded-2xl border border-white/10 shadow-lg group-hover:border-orange-500/30 transition-all duration-500 bg-[#141311]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-80 group-hover:opacity-30 transition-opacity duration-500" />
      </div>
          ...
          {tags.map((tag, index) => {
            ...
            return (
              <span
                ...
                className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-medium text-gray-300 bg-white/[0.04] border border-white/[0.07] backdrop-blur-sm group-hover:border-orange-500/25 group-hover:bg-white/[0.08] group-hover:text-white transition-all duration-300"
              >
                {isObject ? (
                  <>
                    {tag.icon && <span className="text-xs sm:text-sm">{tag.icon}</span>}
                    <span>{tag.name}</span>
                  </>
                ) : (
                  tag
